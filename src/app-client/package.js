...
var profile = {
	...
	resourceTags: {
		...
		// All files in the `app-client/resources` directory that are not CSS files are marked as copy-only, since these
		// files are typically binaries (images, etc.) and may be corrupted by the build system if it attempts to
		// process them and naively assumes they are scripts.
		copyOnly: function (filename, mid) {
			return (/^app-client\/resources\//.test(filename) && !/\.css$/.test(filename));
		},
